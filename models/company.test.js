...
describe("findAll", function () {
  ...
  test("works: max filter", async function () {
  ...
  test("works: min-max filter", async function () {
  ...
  test("works: nothing found", async function () {
    let companies = await Company.findAll({ name: "impossble" });
    expect(companies).toEqual([]);
  });

  test("fail: min>max", async function () {
    try {
...
describe("get", function () {
  test("works", async function () {
    ...
    expect(company).toEqual({
      ...
      logoUrl: "http://c1.img",
      jobs: [
        { id: testJobIds[0], title: "j1", salary: 1, equity: "0.1" },
        { id: testJobIds[1], title: "j1b", salary: 2, equity: "0.2" }
      ]
    });
  });
